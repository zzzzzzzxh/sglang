...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def forward_batch_generation(self, model_worker_batch: ModelWorkerBatch):
        if (
            ...
            if tokenizer is not None:
                try:
                    # draft_token contains all candidate tokens generated by the draft model
                    # verify_input.draft_token is a tensor
                    if verify_input.draft_token is not None:
                        draft_ids = verify_input.draft_token
                        ...
                        print(f"DEBUG: EagleV2 Draft Model Output (Candidates): {decoded_text}", flush=True)
                except Exception as e:
                    print(f"DEBUG: Failed to decode EagleV2 draft tokens: {e}", flush=True)
    ...
    def verify(self, batch: ModelWorkerBatch):
        ...
        if tokenizer is not None:
            try:
                # predict is 1D tensor of all accepted tokens across the batch.
                if isinstance(predict, torch.Tensor):
                    ids_list = predict.cpu().tolist()
                else:
                    ids_list = list(predict)
                
                decoded_text = tokenizer.batch_decode(ids_list)
                print(f"DEBUG: EagleV2 Target Model Output (Final Verified): {decoded_text}", flush=True)

                # Also print the accepted tokens (verified by the target model)
                # accept_index tells us which tokens in 'predict' are actually accepted?
                # No, 'predict' here is the result of tree_speculative_sampling_target_only which puts the accepted path tokens into 'predict'.
                # But let's clarify 'accept_index'.
                # In eagle_info_v2.py, tree_speculative_sampling_target_only fills 'predict' and 'accept_index'.
                # 'predict' seems to hold the accepted tokens.
                # 'accept_index' seems to hold the indices in the flattened candidate list that were accepted.
                
                # If the user wants the "base model verify result", it essentially IS the accepted tokens.
                # The 'logits_output' is from the target model verify pass.
                # The sampling step decides which ones to keep.
                
                # So 'predict' contains the tokens that were accepted from the draft + 1 new token from target.
                
                # If we want to see exactly what the target model "verified" (accepted), it is 'predict'.
                
            except Exception as e:
                print(f"DEBUG: Failed to decode EagleV2 tokens: {e}", flush=True)
